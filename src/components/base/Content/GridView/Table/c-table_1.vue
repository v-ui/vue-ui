...
export default {
  ...
  mounted() {
    this.injectionHover(
      this.$refs.fixedTableBody.children[0].children[1],
      this.$refs.activeTableBody.children[0].children[1]
    );
    this.injectionHover(
      this.$refs.activeTableBody.children[0].children[1],
      this.$refs.fixedTableBody.children[0].children[1]
  ...
  methods: {
    ...
    syncScroll: function(xCoord, yCoord) {
      if (this.$refs.fixedTableBody && Math.abs(yCoord) > 1)
        this.$refs.fixedTableBody.scrollTop = yCoord;
      if (this.$refs.activeTableHeader && Math.abs(xCoord) > 1)
        this.$refs.activeTableHeader.scrollLeft = xCoord;
      if (this.$refs.activeTableBody) {
        if (Math.abs(xCoord) > 1)
          this.$refs.activeTableBody.scrollLeft = xCoord;
        if (Math.abs(yCoord) > 1) this.$refs.activeTableBody.scrollTop = yCoord;
      }
      if (this.$refs.activeTableFooter && Math.abs(xCoord) > 1)
        this.$refs.activeTableFooter.scrollLeft = xCoord;
    }
  }
};
</script>
