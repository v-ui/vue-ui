...
export default {
  ...
  computed: {
    now: function() {
      const now = new Date();
      return new Date(
        ...[1970, 1, 1, now.getHours(), now.getMinutes(), now.getSeconds()]
      );
    },
    types: function() {
    ...
    fillInfo: function() {
      if (isNaN(Date.parse(this.dateMin)) && isNaN(Date.parse(this.dateMax)))
        return ``;
      else if (
        !isNaN(Date.parse(this.dateMin)) &&
        !isNaN(Date.parse(this.dateMax))
      )
        return `${this.showInfo(this.dateMin)}~${this.showInfo(this.dateMax)}`;
      else if (isNaN(Date.parse(this.dateMin)))
        return `...~${this.showInfo(this.dateMax)}`;
      else if (isNaN(Date.parse(this.dateMax)))
        return `${this.showInfo(this.dateMin)}~...`;
      return "";
    },
    message: function() {
  ...
  methods: {
    ...
    formatValue: function(value, call) {
      ...
      return s + tools.string.padStart(call, 2, "0");
    },
    ...
    setSelectAreaStart: function(element, type = "left") {
      ...
      if (selectionStart <= this.selectArea.one.end) {
        ...
        selectionStart >= this.selectArea.two.start &&
        selectionStart <= this.selectArea.two.end
      ) {
        start =
          type == "left"
            ? this.selectArea.one.start
      ...
      var timer = setTimeout((vm = this) => {
        ...
        if (d.toString() !== "Invalid Date") {
          if (selectionStart <= vm.selectArea.one.end) {
            ...
            selectionStart >= vm.selectArea.two.start &&
            selectionStart <= vm.selectArea.two.end
          ) {
          ...
          let str = value
            .split(":")
            .map(e => tools.string.padStart(e, 2, "0"))
